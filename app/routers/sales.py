...
def send_sale_receipt_email(
    ...
    logo_url = (getattr(current_user, "business_logo_url", None) or "").strip()
    logo_html = (
        f"<div style='margin-bottom:10px'><img src='{escape(logo_url)}' alt='Logo' style='height:48px;max-width:160px;object-fit:contain'></div>"
        if logo_url
        else ""
    )
    watermark_html = (
