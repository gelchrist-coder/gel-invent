...
def _ensure_critical_auth_schema_sync() -> None:
    ...
    with engine.begin() as conn:
        ...
        # EXCLUSIVE lock on `sales` — to re-run on cold starts. CREATE TABLE only
        # touches the brand-new table, so it can't contend on `sales` reads.
        has_sale_supplies = conn.execute(
            text("SELECT to_regclass('public.sale_supplies')")
        ).scalar()
        if not has_sale_supplies:
            conn.execute(
                text(
        ...
        conn.execute(text("ALTER TABLE products ADD COLUMN IF NOT EXISTS wholesale_price NUMERIC(10,2)"))
        conn.execute(text("ALTER TABLE products ADD COLUMN IF NOT EXISTS wholesale_min_quantity NUMERIC(10,2)"))
