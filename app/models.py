...
class User(Base):
    ...
    supabase_user_id: Mapped[str | None] = mapped_column(String(64), unique=True, index=True, default=None)
    ...
    warehouse_id: Mapped[int | None] = mapped_column(ForeignKey("warehouses.id", ondelete="SET NULL"), index=True, default=None)
...
class Product(Base):
    ...
    measurement_type: Mapped[str] = mapped_column(String(32), default="count")
    allows_fractional_sales: Mapped[bool] = mapped_column(Boolean, default=False)
    quantity_step: Mapped[Decimal] = mapped_column(Numeric(10, 2), default=Decimal("1"))
    variant_group: Mapped[str | None] = mapped_column(String(120), default=None)
    variant_label: Mapped[str | None] = mapped_column(String(120), default=None)
    brand: Mapped[str | None] = mapped_column(String(100), default=None)
    size: Mapped[str | None] = mapped_column(String(64), default=None)
    color: Mapped[str | None] = mapped_column(String(64), default=None)
    shade: Mapped[str | None] = mapped_column(String(64), default=None)
    ...
    # Wholesale tier (only used by businesses with the wholesale_pricing
    # capability): price per piece, and the minimum quantity that qualifies.
    wholesale_price: Mapped[Decimal | None] = mapped_column(Numeric(10, 2), default=None)
    wholesale_min_quantity: Mapped[Decimal | None] = mapped_column(Numeric(10, 2), default=None)
...
class WarehouseStockItem(Base):
    ...
    wholesale_min_quantity: Mapped[Decimal | None] = mapped_column(Numeric(10, 2), default=None)
    image: Mapped[str | None] = mapped_column(Text, default=None)
    variants_json: Mapped[list | None] = mapped_column(JSON, default=list)
    unit_conversions_json: Mapped[list | None] = mapped_column(JSON, default=list)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    )
